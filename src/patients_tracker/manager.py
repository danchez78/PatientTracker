...
def _get_patients_today(message: types.Message):
    logger.info(f"Called `list_today` command from user with ID `{message.from_user.id}`")
...
def _get_patients_for_week(message: types.Message):
    logger.info(f"Called `list_week` command from user with ID `{message.from_user.id}`")
...
def _save_patient(message: types.Message) -> None:
    ...
    logger.info(f"Saving patient with name `{patient_name}` and birth `{patient_birth}`")
